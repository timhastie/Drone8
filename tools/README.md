...
persistence and most of the main canvas of `LIRA-8.pd` (~7,400 objects)
...
- `pd_parse.py` — minimal .pd parser used for validation (index-range
  ...
  before "cvs=parse".
- `base.pd` — snapshot of LIRA-8.pd *before* the generated block
  (post-vibrato, pre-sequencer). The generator's input.

## Regenerating
```bash
python3 generate_sequencer.py   # reads base.pd, writes work.pd
cp work.pd ../LIRA-8_Plugin/LIRA-8/LIRA-8.pd
```
Then deploy: copy LIRA-8.pd (and any changed abs/*.pd) into all FIVE
...
`pd -nogui -noaudio -noprefs -path abs LIRA-8.pd harness.pd`, where the
